...
- `compatibility.js`: native model/year selectors, optional variant/location/system details and prefilled text/WhatsApp enquiries. Data remains in page memory, with no upload, tracking or persistent storage. A dashboard photo is attached by the customer in their messaging app, not on this site. Header, hero and mobile compatibility actions jump to `#compatibility`.
