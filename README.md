...
The approved European owner experience is `index.html`; the Turo/fleet page is `hosts.html`. Shared runtime files: `styles.css`, `app.js`, `vehicle-catalog.js`. Static GitHub Pages, no build dependency. Legacy scripts/assets remain available but are no longer loaded by the new pages.
...
- The guest guide is a clearly labeled iPhone DRAFT: installer-confirmed MENU/pairing/AUX/CarPlay steps, but host identity and tutorial QR remain pending. `guest-guide.html` is noindex and excluded from the sitemap. Its PDF is a public sample, not a finalized guest handout.
